...
class GeoMag:
    ...
    def __init__(self, wmm_filename=None):
        ...
        self.snorm = [z[0:13], z[0:13], z[0:13], z[0:13], z[0:13], z[0:13], z[0:13], z[0:13], z[0:13], z[0:13], z[0:13],
                      z[0:13], z[0:13]]
        self.snorm[0][0] = 1.0
    ...
    def geomag(self, dlat, dlon, h=0,
               time=datetime.today()):
        # latitude (decimal degrees), longitude (decimal degrees), altitude (meters), date
        ...
        if alt != oalt or glat != olat:
            ...
            ca = (alt + d) / r  # == cd. checked
            sa = self.c2 * crlat * srlat / (r * d)  # == sd. checked
    ...
    def geomag_xyz_iter(self, lla_array, t_array):  # (dlat, dlon, alt) array, t array
        results = []
        for (dlat, dlon, alt), t in zip(lla_array, t_array):
            ans = self.geomag(dlat, dlon, alt, t)
            results.append([ans.bx, ans.by, ans.bz])
